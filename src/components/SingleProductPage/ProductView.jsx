...
export default function ProductView({
  ...
  const selectVarient = (variant, item) => {
    if (varients.length > 0) {
      const replacePrice = getFirstVarients.map((v) => {
        if (parseInt(v.product_variant_id) === parseInt(variant.id)) {
          return item;
        }
        return v;
      });
  ...
  // Tailoring services state
  const [selectedTailoring, setSelectedTailoring] = useState("Semi Stitched");
  const [selectedSize, setSelectedSize] = useState("32\"");

  return (
    <>
      <div className={`product-view w-full lg:flex justify-between h-full ${className || ""}`}>
        ...
        <div className="flex-1">
          <div className="product-details w-full mt-10 lg:mt-0">
            ...
            <h1 className="text-xl font-medium text-qblack mb-4">
              {product.name}
            </h1>
            ...
            <div className="flex space-x-2 items-baseline mb-7">
              <span
            ...
            {/* SKU and Shipping Info */}
            <div className="mb-4">
              <p className="text-sm text-qgray mb-1">
                <span className="font-medium">SKU:</span> {product.sku || 'N/A'}
              </p>
              <p className="text-sm text-qgreen font-medium">
                <span className="text-qblack">Shipping:</span> FREE (Limited Period)
              </p>
            </div>
            ...
            {varients.length > 0 &&
              varients.map((variant) => (
                <div key={variant.id} className="mb-6">
                  <h3 className="text-sm font-medium text-qblack mb-3 capitalize">
                    {variant.name}:
                  </h3>
                  <div className="flex flex-wrap gap-2">
                    {variant.active_variant_items.map((item) => (
                      <button
                        key={item.id}
                        onClick={() => selectVarient(variant, item)}
                        className={`px-4 py-2 border rounded-md text-sm ${getFirstVarients.some(v => v.id === item.id)
                          ? 'border-qpurple bg-qpurplelow/10 text-qpurple'
                          : 'border-qgray text-qblack hover:border-qpurple'
                          }`}
                      >
                        {item.name}
                      </button>
            ...
            {/* Report Button */}
            <div className="flex space-x-2 items-center mb-[20px] report-btn">
              <span>
                <svg
                  width="12"
                  height="13"
